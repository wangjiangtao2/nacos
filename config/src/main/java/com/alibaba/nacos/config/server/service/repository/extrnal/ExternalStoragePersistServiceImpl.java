...
public class ExternalStoragePersistServiceImpl implements PersistService {
    ...
    public Page<ConfigInfo> findAllConfigInfo(final int pageNo, final int pageSize, final String tenant) {
        ...
        String sqlFetchRows = "";
        if (PropertyUtil.isUsePostgreSql()) {
    ...
    public Page<ConfigInfoWrapper> findAllConfigInfoForDumpAll(final int pageNo, final int pageSize) {
        ...
        String sqlFetchRows = "";
        if (PropertyUtil.isUsePostgreSql()) {
            sqlFetchRows = " SELECT t.id,type,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified "
                + " FROM ( SELECT id FROM config_info   ORDER BY id LIMIT offset ? limit ?  )"
                + " g, config_info t WHERE g.id = t.id ";
        } else {
            sqlFetchRows = " SELECT t.id,type,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified "
                + " FROM ( SELECT id FROM config_info   ORDER BY id LIMIT ?,?  )"
    ...
    public Page<ConfigInfoWrapper> findAllConfigInfoFragment(final long lastMaxId, final int pageSize) {
        ...
        if (PropertyUtil.isUsePostgreSql()) {
            // 改 postgre 查询
            select =
                "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,type from config_info where id > ? order by id asc offset  ? limit ? ";
        } else {
            select = "SELECT id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,type from config_info where id > ? order by id asc limit ?,?";
        }
    ...
    public Page<ConfigInfoBetaWrapper> findAllConfigInfoBetaForDumpAll(final int pageNo, final int pageSize) {
        ...
        String sqlFetchRows = "";
        if (PropertyUtil.isUsePostgreSql()) {
            sqlFetchRows = " SELECT t.id,data_id,group_id,tenant_id,app_name,content,md5,gmt_modified,beta_ips "
        ...
        if (PropertyUtil.isUsePostgreSql()) {
            sqlFetchRows =
                "select data_id,group_id,tenant_id,datum_id,app_name,content from config_info_aggr where data_id=? and "
                    + "group_id=? and tenant_id=? order by datum_id limit offset ? limit ? ";
        } else {
            sqlFetchRows =
                "select data_id,group_id,tenant_id,datum_id,app_name,content from config_info_aggr where data_id=? and "
    ...
    public List<ConfigInfoWrapper> listGroupKeyMd5ByPage(int pageNo, int pageSize) {
        ...
        String sqlFetchRows = "";
        if (PropertyUtil.isUsePostgreSql()) {
            sqlFetchRows = " SELECT t.id,data_id,group_id,tenant_id,app_name,md5,type,gmt_modified FROM "
